...
#include"vksandbox.h"//for messagebox
#include<stdio.h>
#include<stdlib.h>
#include<stdarg.h>
#include<string.h>
...
int log_error(const char *file, int line, int quit, const char *format, ...)
{
	...
	printed+=vsnprintf(latest_error_msg+printed, G_BUF_SIZE-printed, format, args);
	va_end(args);
...
ArrayHandle load_file(const char *filename, int bin, int pad, int quitifnotfound)
{
	struct stat info={0};
	FILE *f;
	ArrayHandle str;
